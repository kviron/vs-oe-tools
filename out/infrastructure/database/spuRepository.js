...
exports.getSpuEditorOptions = getSpuEditorOptions;
...
async function getSpuEditorOptions(preferredPackageName, spuId) {
    return (0, projectDatabaseSession_1.withProjectDatabaseSession)(async ({ client, options }) => {
        ...
        const typesResult = await (0, databaseQueryExecutor_1.executeMonitoredQuery)(client, {
        ...
        const types = typesResult.rows.map(row => ({ id: Number(row.id), name: row.name }));
...
async function updateSpu(id, draft, log = () => undefined) {
    if (!Number.isSafeInteger(id) || id <= 0) {
        throw new Error('Указан некорректный ID SPU.');
    }
    (0, spuCreation_1.validateSpuDraft)(draft);
    const sqlScript = encodeWindows1251(draft.sqlScript, 'SQL-скрипт');
    const comment = encodeWindows1251(draft.comment, 'Комментарий');
...
async function loadSpuRecord(client, database, id, forUpdate = false) {
    const result = await (0, databaseQueryExecutor_1.executeMonitoredQuery)(client, {
        ...
        values: [id, spuCreation_1.spuClassId], source: `Загрузка SPU ${id}`, database,
    });
    const row = result.rows[0];
    if (!row) {
        throw new Error(`SPU ${id} не найден.`);
    }
...
async function allocateAutomaticId(client, database, rangeStart, purpose) {
    const result = await (0, databaseQueryExecutor_1.executeMonitoredQuery)(client, {
    ...
    if (!Number.isSafeInteger(id) || id <= 0) {
        throw new Error(`В диапазоне ${rangeStart} нет свободного ID для ${purpose}.`);
    }
    return id;
}
async function allocateDeveloperId(client, database, range, purpose) {
    const result = await (0, databaseQueryExecutor_1.executeMonitoredQuery)(client, {
...
//# sourceMappingURL=spuRepository.js.map