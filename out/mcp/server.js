...
(0, arguments_1.readArgument)('--workspace');
(0, arguments_1.readRoleArgument)();
const server = new McpServer({ name: 'vc-ve-tools-database', version: '0.22.0' }, {
    instructions: [
        ...
        'For East Express REST testing, call start_http_test_server with one exact method name or * for all methods, use call_http_test_server for requests, and always call stop_http_test_server in cleanup even when a request fails.',
        ...
        'Direct SQL access is read-only. Controlled mutations are available only through update_method_source, create_class_method, and explicitly confirmed update_database, update_packages, and update_binaries commands in VS Code. Project updates run in a visible terminal. Include relevant object IDs in analysis so navigation can continue.',
    ].join(' '),
});
