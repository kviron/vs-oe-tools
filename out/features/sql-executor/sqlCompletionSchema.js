...
exports.buildSqlCompletionSchema = buildSqlCompletionSchema;
exports.completeAliasColumns = completeAliasColumns;
function buildSqlCompletionSchema(rows) {
    ...
    const schema = Object.fromEntries(Array.from(schemas, ([schemaName, tables]) => [schemaName, Object.fromEntries(tables)]));
    return {
        schema,
        defaultSchema: schemas.has('public') ? 'public' : schemas.keys().next().value,
    };
}
function completeAliasColumns(document, position, completion) {
...
function findTableReference(statement, qualifier, completion) {
    ...
    for (const match of statement.matchAll(pattern)) {
        ...
        if (alias.toLocaleLowerCase('en-US') !== qualifier && table.toLocaleLowerCase('en-US') !== qualifier) {
            continue;
        }
...
function findTableColumns(completion, schemaName, tableName) {
