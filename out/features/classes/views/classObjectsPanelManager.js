...
async function openClassObjects(context, classId, objectId) {
    ...
    const performLoad = async (offset = 0, targetObjectId) => {
        ...
        try {
            ...
            const columnSettings = append ? undefined : (0, classObjectColumnSettings_1.normalizeClassObjectColumnSettings)(result.columns.map(column => column.key), context.workspaceState.get((0, classObjectColumnSettings_1.classObjectColumnSettingsKey)(classId)));
            panel.title = `Справочник — ${result.className}`;
            await panel.webview.postMessage({ command: 'classObjectsLoaded', result, append, columnSettings });
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        ...
        if (message.command === 'saveClassObjectColumnSettings') {
            ...
            const settings = (0, classObjectColumnSettings_1.normalizeClassObjectColumnSettings)(availableKeys, message.settings);
            await context.workspaceState.update((0, classObjectColumnSettings_1.classObjectColumnSettingsKey)(classId), settings);
            return;
        }
