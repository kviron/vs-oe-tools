...
const packageSyncDiffRepository_1 = require("../infrastructure/database/packageSyncDiffRepository");
...
async function activate(context) {
    ...
    const activeDatabaseSelectionPath = (0, databaseSelection_1.getActiveDatabaseSelectionPath)();
    const publishActiveDatabaseSelection = async () => {
        const currentWorkspacePath = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!currentWorkspacePath) {
            return;
        }
        const profile = vscode.workspace.getConfiguration('vcVeTools').get(constants_1.databaseProfileSetting, '');
        await (0, databaseSelection_1.writeDatabaseSelection)(activeDatabaseSelectionPath, currentWorkspacePath, profile);
    };
    ...
    const methodEditor = (0, methodEditorProvider_1.registerMethodEditor)(context, async (draft, target) => {
        let databaseOptions;
        if (target) {
            const currentWorkspacePath = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (!currentWorkspacePath) {
            ...
            databaseOptions = await (0, projectDatabaseOptions_1.getProjectDatabaseOptionsForDatabase)(currentWorkspacePath, target.database, target.host);
        }
        else {
            databaseOptions = await (0, projectDatabaseOptions_1.getProjectDatabaseOptions)();
        }
    ...
    const explorerProvider = new explorerViewProvider_1.ExplorerViewProvider(context.workspaceState, context.extensionUri, classRepository_1.loadClasses, (id, pinned) => (0, classDetailsPanelManager_1.openClassDetails)(context, methodEditor, id, pinned), id => dfmEditor.open(id), id => (0, dfmPreview_1.openDfmPreview)(context, id), objectSearchRepository_1.searchDatabaseObjects, id => methodEditor.open(id), id => (0, attributeDetailsPanelManager_1.openAttributeDetails)(context, id), id => (0, classObjectsPanelManager_1.openClassObjects)(context, id), id => (0, objectViewPanelManager_1.openObjectView)(context, id), id => (0, entityPropertiesPanelManager_1.openEntityProperties)(context, id), packageExplorerRepository_1.loadPackages, packageExplorerRepository_1.loadPackageTree, packageExplorerRepository_1.loadPackageFileContent, (fileId, objectId) => (0, packageContentPanelManager_1.openPackageContent)(context, fileId, objectId, async (id, kind) => {
        if (kind === 'class') {
            ...
            await (0, objectViewPanelManager_1.openObjectView)(context, id);
        }
    }));
    ...
    const navigationActions = {
        ...
        executeLifecycleMethod: async (methodId, methodParameter, database, host) => {
            ...
            const result = await (0, oeStaticMethodExecutor_1.executeOeStaticMethod)(workspacePath, methodId, methodParameter, database, host, await getClientCredentials());
        ...
        getProductionTask: async (query, limit) => {
            ...
            const tasks = await (0, productionTasksRepository_1.loadProductionTasksByQuery)(options, query, limit, productionTasksLogger);
            return {
                database: options.database,
                query,
                count: tasks.length,
    ...
    const nativeLogEditor = new nativeLogEditorProvider_1.NativeLogEditorProvider();
    const nativeLogEditorRegistrations = nativeLogEditor.registrations();
    const nativeLogsProvider = new nativeLogsViewProvider_1.NativeLogsViewProvider(context.extensionUri, fileName => nativeLogEditor.open(fileName));
    ...
    const configurationListener = vscode.workspace.onDidChangeConfiguration(async (event) => {
        ...
        if (event.affectsConfiguration(`vcVeTools.${constants_1.databaseRoleSetting}`) || event.affectsConfiguration(`vcVeTools.${constants_1.databaseProfileSetting}`)) {
            ...
            (0, packageContentPanelManager_1.closePackageContentPanels)();
