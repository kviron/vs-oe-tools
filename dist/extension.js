...
// src/infrastructure/configuration/projectDatabaseOptions.ts
...
function getDatabaseRole() {
  return vscode.workspace.getConfiguration("vcVeTools").get(databaseRoleSetting) === "test" ? "test" : "main";
}
...
async function getProjectDatabaseOptions() {
  ...
  let varsContent;
  try {
    varsContent = iconv.decode(await (0, import_promises.readFile)(varsPath.fsPath), "win1251");
  } catch {
...
async function getClassMethods(classId, className, includeInherited) {
  ...
  try {
    ...
    const text = includeInherited ? `WITH RECURSIVE class_chain AS (
			   ...
			   ORDER BY chain.depth, lower(method.name), method.id` : `SELECT to_jsonb(method) AS data, owner.name AS ownername, 0 AS depth
    ...
    const visibleMethods = /* @__PURE__ */ new Map();
...
function methodTypeName(value) {
  switch (value) {
    case "1":
      return "\u041E\u0431\u044A\u0435\u043A\u0442\u043D\u044B\u0439";
    case "2":
      return "\u0412\u043D\u0435\u0448\u043D\u044F\u044F \u043F\u0440\u043E\u0446\u0435\u0434\u0443\u0440\u0430";
    case "3":
      return "\u0418\u043D\u0442\u0435\u0440\u043F\u0440\u0435\u0442\u0438\u0440\u0443\u0435\u043C\u044B\u0439";
    case "4":
      return "Visual Basic";
    case "5":
      return "Java";
...
async function applyProjectEncoding(context, enabled) {
  let previousEncodings = context.workspaceState.get(previousEncodingsKey);
  if (enabled && !previousEncodings) {
    previousEncodings = sourceLanguageIds.map((languageId) => {
      const encoding = vscode2.workspace.getConfiguration("files", { languageId }).inspect("encoding")?.workspaceLanguageValue;
      return {
        languageId,
        hadValue: encoding !== void 0,
        value: encoding
      };
    });
  ...
  for (const languageId of sourceLanguageIds) {
    const configuration = vscode2.workspace.getConfiguration("files", { languageId });
    const previousEncoding = previousEncodings?.find((item) => item.languageId === languageId);
    const encoding = enabled ? "windows1251" : previousEncoding?.hadValue ? previousEncoding.value : void 0;
  ...
  if (!enabled) {
    await context.workspaceState.update(previousEncodingsKey, void 0);
  }
}
...
async function openClassDetails(context, methodEditor, id, pinned, activeTab = "class") {
  ...
  const previewPanel = previousPreviewPanelId === void 0 ? void 0 : classDetailPanels.get(previousPreviewPanelId);
  if (previewPanel && previousPreviewPanelId !== void 0 && !previewPanel.pinned) {
...
async function getSessionContext(client, databaseName) {
  ...
  const computerName = (0, import_node_os.hostname)();
  ...
  try {
    ...
    const logResult = await executeMonitoredQuery(client, {
      ...
      values: [
        ...
        // ObjID - ID метода
        5,
        // ObjClassID - класс метода
        2,
        // ChangeType - изменение существующего объекта
        newValues,
        // NewValues - сериализованные новые значения
        sessionContext.userId,
        // UserID - ID пользователя
        sessionContext.computerName,
        // ComputerName - имя компьютера
        lastChange,
        // ChangeDate - дата изменения
        oldValues,
        // OldValues - сериализованные старые значения
        "",
        // TransactionComment - пустой комментарий
        /* @__PURE__ */ new Date("1899-12-30"),
        // VersionObject - нулевая дата (версия объекта)
        method.seniorId,
        // RootObjID - ID родительского класса (SeniorID)
        3
        // RootObjClassID - класс класса
      ],
...
// src/application/extensionApplication.ts
