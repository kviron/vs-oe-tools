...
var SettingsItem = class extends vscode.TreeItem {
  constructor(enabled) {
...
var SettingsProvider = class {
  ...
  setProjectRootEnabled(enabled) {
    this.projectRootItem.checkboxState = enabled ? vscode.TreeItemCheckboxState.Checked : vscode.TreeItemCheckboxState.Unchecked;
...
function isOpenClassMessage(message) {
  return typeof message === "object" && message !== null && "command" in message && "id" in message && "pinned" in message && message.command === "openClass" && typeof message.id === "number" && typeof message.pinned === "boolean";
}
...
async function applyProjectEncoding(context, enabled) {
  ...
  if (enabled && !previousEncodings) {
    previousEncodings = sourceLanguageIds.map((languageId) => {
      const encoding = vscode.workspace.getConfiguration("files", { languageId }).inspect("encoding")?.workspaceLanguageValue;
      return {
        languageId,
