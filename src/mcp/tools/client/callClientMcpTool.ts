...
import { callManagedClientMcpTool } from '../../client/lifecycle';
import { z } from '../../schemas';
import type { McpToolServer } from '../../toolTypes';

export function registerTool(server: McpToolServer): void {
	server.registerTool('call_client_mcp_tool', {
		description: 'DEPRECATED: this potentially data-changing gateway will be removed soon. Call a tool exposed by an already running East Express client HTTP MCP server. Call start_client_mcp, then list_client_mcp_tools, and always stop_client_mcp when finished. Some client tools create or save objects, so invoke mutating tools only when the user explicitly requests that action.',
		inputSchema: {
