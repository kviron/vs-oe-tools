...
export const dfmSourceQuery = sql`WITH RECURSIVE class_chain AS (
...
export const dfmInheritanceQuery = sql`WITH RECURSIVE class_chain AS (
...
export function formatDfmSource(row: DfmSourceRow, startLine?: number, maxLines?: number): Record<string, unknown> {
	const text = decodeSourceValue(row.defvalue);
	return {
		...
		source: createSourceExcerpt(text, startLine, maxLines),
	};
}
