export interface PkfObjectProperty {
	attributeId: number;
	name: string;
	value: string;
	format: 'scalar' | 'block';
...
export interface PkfDatabaseObject {
	id: number;
	className: string;
	name: string;
	properties: PkfObjectProperty[];
}
...
export function createEmptyPkf(autogroup: string | null | undefined, newline = '\r\n'): string {
	const lines = ['file'];
	if (autogroup?.trim()) {lines.push(`  autogroup '${escapePkfString(autogroup.trim())}';`);}
	lines.push('data', 'end.');
	return `${lines.join(newline)}${newline}`;
}

export function parseSerializedAttributeValues(source: string): SerializedAttributeValue[] {
	const fields = parseCsvFields(source);
	if (fields.length % 2 !== 0) {throw new Error('Журнал содержит непарное количество ID атрибутов и значений.');}
	const result: SerializedAttributeValue[] = [];
	for (let index = 0; index < fields.length; index += 2) {
		const attributeId = Number(fields[index]);
		if (!Number.isSafeInteger(attributeId)) {throw new Error(`Некорректный ID атрибута в журнале: ${fields[index] ?? ''}.`);}
...
export function appendPkfObjects(source: string, objects: readonly PkfDatabaseObject[]): string {
	...
	if (!finalMarker || finalMarker.index === undefined) {throw new Error('В PKF не найден завершающий маркер end.');}
	if (!objects.length) {return source;}
	...
	if (existingBlocks.some(block => !Number.isSafeInteger(block.id))) {throw new Error('Не удалось определить ID одного из объектов PKF.');}
	...
	const insertions = new Map<number, PkfDatabaseObject[]>();
...
export function serializePkfObject(object: PkfDatabaseObject, newline = '\n'): string {
	if (!object.className.trim()) {throw new Error(`Для объекта ID ${object.id} не определено имя класса.`);}
	return [
...
function serializePkfProperty(property: PkfObjectProperty, newline: string): string {
	const name = property.name.toLocaleLowerCase('ru') === 'comment' ? `$${property.name}` : property.name;
	if (property.format === 'scalar') {return `    ${name} = '${escapePkfString(property.value)}';`;}
...
function formatPkfObjectName(value: string): string {
	if (!value || value === '$') {return '$';}
	return /^[\p{L}_][\p{L}\p{N}_]*$/u.test(value) ? value : `$"${value.replace(/"/g, '""')}"`;
}

function parseCsvFields(source: string): string[] {
	const fields: string[] = [];
	let value = '';
	let quoted = false;
	for (let index = 0; index < source.length; index++) {
		...
		if (quoted) {
			if (character === '"' && source[index + 1] === '"') {value += '"'; index++; continue;}
			if (character === '"') {quoted = false; continue;}
			value += character;
			continue;
		}
		if (character === '"' && value.length === 0) {quoted = true; continue;}
		if (character === ',') {fields.push(value); value = ''; continue;}
		value += character;
	}
	if (quoted) {throw new Error('В журнале не закрыта двойная кавычка.');}
	fields.push(value);
	return fields;
}
