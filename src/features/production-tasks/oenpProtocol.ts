...
interface Field { name: string; type: number }
export type MemoryDataRow = Record<string, number | string | null>;

const requestTrailer = Buffer.from(
	'00000000000000000c2000000100000000000000020000000c20000001000000' +
...
export function createOenpPacket(requestId: number, operation: Buffer, body = Buffer.alloc(0)): Buffer {
...
export function createInitialPacket(clientSessionKey: string): Buffer {
...
export function extractClientSessionKey(capture: Buffer): string | undefined {
	const prefix = Buffer.from('4f454e501700000001000000019d0000', 'hex');
	const offset = capture.indexOf(prefix);
	if (offset < 0 || offset + 32 > capture.length) { return undefined; }
	return capture.subarray(offset + prefix.length, offset + prefix.length + 16).toString('hex');
}

export function extractCurrentPersonId(capture: Buffer): number | undefined {
...
export function extractCapturedAuthorization(capture: Buffer): CapturedAuthorization | undefined {
	const decoded = iconv.decode(capture, 'win1251');
	...
	while ((offset = capture.indexOf('OENP', offset, 'ascii')) >= 0) {
		if (offset + 13 <= capture.length && capture.readUInt32LE(offset + 8) === 6 && capture[offset + 12] === 0) {
			...
			if (packetLength >= 13 && offset + packetLength <= capture.length) {
				...
				if (match) { challenge = match[0].toUpperCase(); break; }
			}
		}
		offset += 4;
	}
...
export function createClientVersionPacket(requestId: number): Buffer {
...
export function createProtocolInitPacket(requestId: number): Buffer {
	return createOenpPacket(requestId, Buffer.from('fdf001', 'hex'), Buffer.from('0700000003000000', 'hex'));
}

export function createDatabaseProbePacket(requestId: number): Buffer {
	return createOenpPacket(requestId, Buffer.from('fd9801', 'hex'), Buffer.from('080100000003000000', 'hex'));
}

export function createClientReadyPacket(requestId: number): Buffer {
	return createOenpPacket(requestId, Buffer.from('fdf401', 'hex'), Buffer.from([0]));
}

export function createChallengePacket(requestId: number): Buffer {
	return createOenpPacket(requestId, Buffer.from([0x07]), Buffer.from([0x07, 0, 0, 0, 3, 0, 0, 0]));
}

export function parseChallenge(packet: Buffer): string {
	const match = iconv.decode(packet.subarray(13), 'ascii').match(/[A-F\d]{32}/i);
	if (!match) { throw new Error(readOenpError(packet) || 'Сервер не вернул ключ авторизации.'); }
	return match[0].toUpperCase();
}

export function createLoginPacket(requestId: number, parameters: string): Buffer {
	const value = iconv.encode(parameters, 'win1251');
...
export function createReadonlyQueryPacket(requestId: number, sql: string, personId: number): Buffer {
...
export function expectedPacketLength(header: Buffer): number {
...
export function parseMemoryDataPacket(packet: Buffer): MemoryDataRow[] {
...
function packedLength(value: number): Buffer {
	...
	if (value <= 0xffff) { const result = Buffer.alloc(3); result[0] = 0xfd; result.writeUInt16LE(value, 1); return result; }
	return Buffer.concat([Buffer.from([0xff]), uint32(value)]);
}

function uint32(value: number): Buffer { const result = Buffer.alloc(4); result.writeUInt32LE(value); return result; }

export function readOenpError(packet: Buffer): string {
	const decoded = iconv.decode(packet.subarray(13), 'win1251').replace(/[\x00-\x1f]+/g, ' ').trim();
